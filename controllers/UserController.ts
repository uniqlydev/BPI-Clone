...
import createPool from "../model/database";

const pool = createPool("app_user");
// Load dotenv
...
exports.login = (req: LoginRequest & Request, res: Response) => {
  ...
  const userQuery =
    "SELECT id, password FROM public.users WHERE email = $1 AND role = 'user' LIMIT 1;";
  ...
  pool.query(
    userQuery,
    values,
    async (err: string, result: { rows: any[] }) => {
      if (err) {
        if (process.env.ENV === "debug") {
          console.error(err);
        }
      ...
      if (result && result.rows && result.rows.length > 0) {
        ...
        const cleanedPassword =
          InputCleaner.cleanPassword(req.body.password) || "";
...
exports.uploadImage = async (
  ...
  const pngMagicNumbers = Buffer.from([
  ...
  if (
    new Uint8Array(buffer.slice(0, pngMagicNumbers.length)).equals(
      pngMagicNumbers
    )
  ) {
    fileType = "PNG";
  } else if (
    new Uint8Array(buffer.slice(0, jpgMagicNumbers.length)).equals(
      jpgMagicNumbers
    )
  ) {
    fileType = "JPG";
  } else if (
    new Uint8Array(buffer.slice(0, jpegMagicNumbers1.length)).equals(
      jpegMagicNumbers1
    ) &&
    ...
    new Uint8Array(buffer.slice(0, jpegMagicNumbers2.length)).equals(
      jpegMagicNumbers2
    ) &&
...
exports.updateProfile = async (req: Request, res: Response) => {
  ...
  const query =
    "UPDATE public.users SET first_name = $1, last_name = $2, phone_number = $3 WHERE email = $4";
  const values = [
    InputCleaner.cleanName(firstName),
    InputCleaner.cleanName(lastName),
    InputCleaner.cleanChequeNumber(phoneNumber),
    InputCleaner.cleanEmail(req.session.user.email),
  ];
