...
exports.login = (req: any, res: any) => {
  ...
  const query =
    "SELECT * FROM users WHERE email = $1 AND role IN ('superuser', 'transacad', 'acctad') LIMIT 1";
...
exports.createCheque = async (req: any, res: any) => {
  ...
  if (!req.session.user || req.session.user.userType !== "transacad") {
    ...
    console.log(
      "this is from createCheque" + req.session.user + req.session.user.userType
    );
  ...
  const query =
    "INSERT INTO cheques (chequenum, amount, date) VALUES ($1, $2, $3)";
...
exports.updateUserStatus = (req: any, res: any) => {
  ...
  pool.query(
    query,
    [InputCleaner.cleanStatus(status), userid],
